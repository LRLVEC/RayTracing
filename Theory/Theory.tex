    ...
    \section{理论推导}
        ...
        \subsection{光线效果的模拟}
            ...
            \subsubsection{镜面反射}
                ...
                \indent 而修正为:
                \begin{equation}
                    t'=\frac{\left(1+\frac{\cos{\left(i_1+i_0\right)}}{\cos{\left(i_1-i_0\right)}}^2\right)
                    \left(\frac{\sin{\left(i_1-i_0\right)}}{\sin{\left(i_1+i_0\right)}}\right)^2}{2}t.
                \end{equation}
                注意到当$i_1$和$i_0$都趋于0时,数值计算可能不精确,于是求极限:
                \begin{equation}
                    \lim_{i_1,i_0\rightarrow 0}\frac{\left(1+\frac{\cos{\left(i_1+i_0\right)}}{\cos{\left(i_1-i_0\right)}}^2\right)
                    \left(\frac{\sin{\left(i_1-i_0\right)}}{\sin{\left(i_1+i_0\right)}}\right)^2}{2}=
                    \left(\frac{n-1}{n+1}\right)^2
                \end{equation}
            \subsubsection{折射}
                这里需要考虑$\overrightarrow{n_{hit}}$与$\overrightarrow{n_0}$的方向一致性.由折射定律
                \begin{equation}
                    \sin{i_0}=n\sin{i_1}
                \end{equation}
                可以求出$i_1$.经过计算得到:\\
                ...
                \indent 计算菲涅尔透射率:
                \begin{equation}
                    t'=\frac{\cos{i_1}\cos{i_0}\left(\frac{2n\sin{i_1}}{\sin{\left(i_0+i_1\right)}}\right)^2
                    \left(1+\frac{1}{\cos^2\left(i_1-i_0\right)}\right)}{2}t.
                \end{equation}
                同理,在$i_1$和$i_0$都趋于0时,计算极限:
                \begin{equation}
                    \lim_{i_1,i_0\rightarrow 0}\frac{\cos{i_1}\cos{i_0}\left(\frac{2n\sin{i_1}}
                    {\sin{\left(i_0+i_1\right)}}\right)^2\left(1+\frac{1}{\cos^2\left(i_1-i_0\right)}\right)}{2}=
                    \left(\frac{2n^2}{1+n}\right)^2.
                \end{equation}
            \subsubsection{点光源漫反射}
                这里采用简化的模型:若交点与点光源之间没有其他图元之间遮挡,则认为可以被光源照到.因此,
                首先计算出相交位置
                $\overrightarrow{p_0}'=\overrightarrow{p_0}+\overrightarrow{n_0}\left(t-\delta\right)$
                ($t$减少$\delta$是因为防止面元后侧的光源干扰)然后对所有光源逐一判断是否有图元遮挡.
\end{document}